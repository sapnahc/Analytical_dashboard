...
<body>
    ...
    <table>
        ...
        if(!empty($subcamp))
        {
             $sql = "select date,sum(clicks) as clicks,sum(imp)as imp,$column from $client where campaign = '$subcamp' group by date,$column order by date asc" ;
        }
        else
        {
             $sql = "select date,sum(clicks) as clicks,sum(imp)as imp,$column from $client group by date,$column order by date asc" ;
        }
...
<script>
  ...
  var subcamp = <?php echo json_encode($subcamp);?>;
    ...
    downloadLink.click();
    // alert("EE");
    window.location.replace("data.php?client="+client+","+subcamp);
    }
